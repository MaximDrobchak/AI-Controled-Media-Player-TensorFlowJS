...
import { Subject, fromEvent, fromPromise } from 'rxjs';
  ...
  render () {
    return (
      <div className='App-intro'>
        ...
        <ul>
          {this.state.todos.map(({ text, done }, index) => {
            return (
              <li key={index}>
                <input
                  onChange={() => updateTodo.next({ index, done: !done })}
                  checked={done}
                  type='checkbox'
                  value=''
                />
                {
                  done === true ? <strike>{text}</strike> :
                  text}
        ...
        <button type='button' onClick={() => fromEvent()}>
          fromEvent
        </button>
      </div>
    );
